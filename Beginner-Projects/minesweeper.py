...
class Board: 
    ...
    def make_new_board(self):
        ...
        # we should construct the list of lists here (or whatever representation
        # you prefer, but since we have a 2-d board, list of list is most natural)
    ...
    def assign_values_to_board(self):
        ...
        # represtents how many neighboring bombs there are. we can precompute these and it'll save us
        # som effort checking what's around the board later on :)
    ...
    def __str__(self):
        ...
        # figure out max width of each column (for alignment)
        widths = []
        for i in range(self.dim_size):
            widths.append(
                max(len(visible_board[row][i]) for row in range(self.dim_size))
            )
        
        # build string
        string_rep = ''
        indices = [str(i).rjust(widths[i]) for i in range(self.dim_size)]
        string_rep += ' ' + ' '.join(indices) + '\n'

        for row in range(self.dim_size):
            row_cells = []
            for col in range(self.dim_size):
                row_cells.append(visible_board[row][col].rjust(widths[col]))
            string_rep += f'{row:>2} ' + ' | '.join(row_cells) + '\n'
